...
export default function Card({ element, mediaType }) {
  ...
  const isMoviePresent = useMovieStore.getState().isMoviePresent;
  const appendMovieIdService = useMovieStore.getState().appendMovieId;
  const removeMovieIdService = useMovieStore.getState().removeMovieId;

  const toggleMovie = () => {
    if (isMoviePresent(elementID)) {
      removeMovieIdService(elementID);
      setFaved(!faved);
    } else {
      appendMovieIdService(elementID);
      setFaved(!faved);
    }
  };
